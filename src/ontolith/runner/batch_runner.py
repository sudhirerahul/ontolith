...
def _parse_scenario(raw: dict) -> Scenario | None:
    try:
        ...
        console.print(f"[yellow]WARNING: Skipping malformed scenario: {e}[/yellow]")
        return None
...
class BatchRunner:
    ...
    def _load_scenarios(
        ...
        if self.dynamic_scenarios and not scenario_id:
            ...
            try:
                ...
                console.print(f"[yellow]WARNING: Dynamic generation failed: {e}[/yellow]")
    ...
    def run_all(
        ...
        console.rule(f"[bold cyan]Ontolith — Run {run_id}[/bold cyan]")
        ...
        if retail_count and not self.use_retail:
            console.print(
                f"[yellow]WARNING: {retail_count} retail_sales scenario(s) loaded but --retail not set — "
