...
MODES:
  ...
  Control dynamic generation:
    python -m ontolith run --all --dynamic --dynamic-count 3
    python -m ontolith run --all --dynamic --dynamic-categories security compliance
...
def _find_project_root() -> Path:
    ...
    # This file is at: src/ontolith/__main__.py
    # Project root is:  ../../  (two levels up)
    this_file = Path(__file__).resolve()
    project_root = this_file.parent.parent.parent  # up from __main__.py → ontolith → src → root
...
def _print_mode_banner(args: argparse.Namespace) -> None:
    ...
    if args.foundry:
        if foundry_endpoint and foundry_key:
            ...
            console.print(Panel(
                f"[bold red]WARNING: --foundry flag set but credentials missing[/bold red]\n"
                f"Missing: [red]{', '.join(missing)}[/red]\n\n"
                f"Run: [cyan]python -m ontolith setup[/cyan] for instructions.",
        ...
        if openrouter_key:
            ...
            console.print(Panel(
                "[bold yellow]WARNING: --llm flag set but OPENROUTER_API_KEY not found[/bold yellow]\n"
...
def cmd_run(args: argparse.Namespace) -> None:
    ...
    console.print(f"\n[green]HTML report:[/green] [cyan]{report_path}[/cyan]")
    console.print(f"[green]Artifacts:[/green] [cyan]{runs_dir}[/cyan]\n")
...
def cmd_report(args: argparse.Namespace) -> None:
    ...
    console.print(f"[green]Report:[/green] [cyan]{report_path}[/cyan]")
...
def cmd_studio_generate(args: argparse.Namespace) -> None:
    ...
    console.print(Panel(
        ...
        f"Run them: [cyan]python -m ontolith run --category retail_sales --retail[/cyan]",
...
def main() -> None:
    parser = argparse.ArgumentParser(
        prog="ontolith",
        description="Ontolith — Voice AI Release Validation Gate",
    )
